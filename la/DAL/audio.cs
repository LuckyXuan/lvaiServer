...
namespace la.DAL
{
	...
	public partial class audio
	{
        ...
        /// <returns></returns>
        public Model.audio getLastAudio(string tel)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 audio_id,user_telphone,audio_url,audio_time,audio_comment,audio_state from audio ");
            strSql.Append(" where user_telphone=@user_telphone order by audio_time desc ");
            SqlParameter[] parameters = { new SqlParameter("@user_telphone", SqlDbType.VarChar, 20) };
            parameters[0].Value = tel;
            la.Model.audio ad = new la.Model.audio();
            DataSet ds = DbHelperSQL.Query(strSql.ToString(), parameters);
            if (ds.Tables[0].Rows.Count > 0)
            {
                return DataRowToModel(ds.Tables[0].Rows[0]);
